...
class UNet(nn.Module):
    ...
    def create_encoder_block(self, in_channels, features):
        """ Encoder block: Conv2D + BatchNorm2D + ReLU (x2) """
        return nn.Sequential(
            nn.Conv2d(in_channels, features, kernel_size=3, padding=1),  # padding=1로 입력 크기 유지
            nn.BatchNorm2d(features),
            nn.ReLU(inplace=True),
            nn.Conv2d(features, features, kernel_size=3, padding=1),  # padding=1로 입력 크기 유지
            nn.BatchNorm2d(features),
            nn.ReLU(inplace=True)
        )
    
    def create_decoder_block(self, in_channels, features):
        """ Decoder block: Conv2D + BatchNorm2D + ReLU (x2) """
        return nn.Sequential(
            nn.Conv2d(in_channels, features, kernel_size=3, padding=1),  # padding=1로 입력 크기 유지
            nn.BatchNorm2d(features),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # 디코딩 경로 및 스킵 연결 (업샘플링 후 크기 맞추기)
        upconv4_out = self.upconv4(bottleneck)
        upconv4_out = F.interpolate(upconv4_out, size=enc4.shape[2:], mode='bilinear', align_corners=False)  # 크기 일치
        dec4 = torch.cat((upconv4_out, enc4), dim=1)
        dec4 = self.decoder4(dec4)

        upconv3_out = self.upconv3(dec4)
        upconv3_out = F.interpolate(upconv3_out, size=enc3.shape[2:], mode='bilinear', align_corners=False)  # 크기 일치
        dec3 = torch.cat((upconv3_out, enc3), dim=1)
        dec3 = self.decoder3(dec3)

        upconv2_out = self.upconv2(dec3)
        upconv2_out = F.interpolate(upconv2_out, size=enc2.shape[2:], mode='bilinear', align_corners=False)  # 크기 일치
        dec2 = torch.cat((upconv2_out, enc2), dim=1)
        dec2 = self.decoder2(dec2)

        upconv1_out = self.upconv1(dec2)
        upconv1_out = F.interpolate(upconv1_out, size=enc1.shape[2:], mode='bilinear', align_corners=False)  # 크기 일치
...
if __name__ == '__main__':
    model = UNet(in_channels=3, out_channels=2, init_features=64)
